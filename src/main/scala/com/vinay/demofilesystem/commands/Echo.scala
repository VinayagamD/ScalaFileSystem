...
import com.vinay.demofilesystem.files.{Directory, File}
...
class Echo(args: Array[String]) extends Command {
  ...
  override def apply(state: State): State = {
    /*
      ...
      else if multiple args
      {
        ...
        if >
         echo to a file (may create a file uf not there)
        if  >>
           append to a file
        else
  ...
  def getRootAfterEcho(currentDirectory: Directory, path: List[String], contents: String,append: Boolean): Directory = {
    /*
      if path is e,pty, then fail (currentDirectory)
      else if no more things to explore = path.tail.isEmpty
        find the file to create/add content to
        if file not found, create file
        else if the entry is actually a directory, then fai
        else
          replace or append contents to the file
